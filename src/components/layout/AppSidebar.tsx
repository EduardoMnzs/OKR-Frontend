...
export function AppSidebar() {
  ...
  return <Sidebar className="border-r border-border/50 bg-sidebar/50 backdrop-blur-sm">
      ...
      <SidebarContent className="p-2">
        {!isCollapsed && <div className="px-2 py-4">
            <CreateOKRModal>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {menuItems.map(item => <SidebarMenuItem key={item.title}>
                  ...
                  isActive
                }) => `flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 ${isActive ? "bg-primary text-primary-foreground shadow-md" : "hover:bg-muted/50 text-sidebar-foreground"}`}>
                      <item.icon className="w-5 h-5 flex-shrink-0 text-sidebar-foreground" />
                      {!isCollapsed && <span className="flex-1 text-sm font-medium text-sidebar-foreground text-left">
                          {item.title}
                        </span>}
                    </NavLink>
                  </SidebarMenuButton>
